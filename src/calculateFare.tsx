const calculateFare = (tablearray:any, input:any) => {
    ...
    const sortedByDistance = tablearray.slice().sort((a:any, b:any) => Math.abs(a.meters - inputNumber) - Math.abs(b.meters - inputNumber));
  
    const closestFourDistances = sortedByDistance.slice(0, 4).map((item:any) => item.meters);
    const closestFourFares = sortedByDistance.slice(0, 4).map((item:any) => item.fare);
    ...
    const calculatedFare: number = ((((inputNumber-x1)/(x0-x1))*((inputNumber-x2)/(x0-x2))*((inputNumber-x3)/(x0-x3)))*y0)+ 
